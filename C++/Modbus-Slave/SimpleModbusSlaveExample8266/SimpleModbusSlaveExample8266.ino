/*
 *  Test program for Industrial Feather equipped with Huzzah8266 
 *  Version 1.0
...
void loop()
{
  ...
  if (ledState) // set led
  {		  
    digitalWrite(LED_BUILTIN, LOW);   // turn the LED on (LOW is the ground level)
  }  
  else if (ledState == 0) // reset led
  {
    digitalWrite(LED_BUILTIN, HIGH);    // turn the LED off by making setting port to voltage level
